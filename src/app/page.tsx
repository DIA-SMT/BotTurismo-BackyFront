...
export default function DashboardPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (intentData) setIntents(intentData)
      if (origenData) setOrigens(origenData)
      if (actividadData) setActividad(actividadData)
      if (franjaData) setFranjas(franjaData)
      if (interData) setInter(interData)
      if (totalCount !== null) {
        setTotal(totalCount)
        // unique turistas: contar distinct chat_id
        const { data: uniqData } = await supabase
          .from('tourist_interactions')
          .select('chat_id')
          .not('chat_id', 'is', null)
        if (uniqData) {
          const unique = new Set(uniqData.map((r: any) => r.chat_id)).size
          setTotalT(unique)
        }
        // pct internacional
        const { count: intlCount } = await supabase
          ...
          .eq('language', 'en')
        if (intlCount && totalCount > 0) setPct(Math.round((intlCount / totalCount) * 100))
      }
